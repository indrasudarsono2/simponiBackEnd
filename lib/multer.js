...
// Keep each event document type in its own physical directory.
const eventStorage = multer.diskStorage({
  destination: function (req, file, cb) {
    const folder = file.fieldname === 'recommendationFile'
      ? 'recommendation'
      : 'briefing';
    const destination = path.join(__dirname, '../uploads/event', folder);
  ...
  filename: function (req, file, cb) {
    ...
    const prefix = file.fieldname === 'recommendationFile'
      ? 'recommendation'
      : 'briefing';
